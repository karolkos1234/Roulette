...
def pixels(r1: float, r2: float, alpha: float):
    ...
    constant =  np.pi *(2/37)
    diameters = np.linspace(r1, r2 + 0.1, 20)
    
    
    

    for num in range(0, 36, 2):
        angles_new = np.linspace(alpha + num* constant, alpha + (num + 1) * constant, 20 )
...
while run:
    ...
    x_co, y_co = rotate(r_coords,theta_coords,(SCREEN_W / 2, (SCREEN_H / 8) * 3), t * 0.04)
